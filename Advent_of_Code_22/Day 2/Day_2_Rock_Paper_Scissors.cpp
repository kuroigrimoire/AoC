/**
 * @file Day_2_Rock_Paper_Scissors.cpp
 * @author Sable
 * @brief 
 * @version 0.1
 * @date 2022-12-10
...
uint64_t round_score(const char& first, const char& second);
uint64_t true_strat(const char& first, const char& second);

int main(){
    // Variable Declarations
    // std::ifstream read("Day_2_Test_Input.txt");
    std::ifstream read("Day_2_Input.txt");
    char opp, mine;
    std::string line;
    std::stringstream iss;
    uint64_t total_score{0}, true_score{0};
    ...
    while(!read.eof()){
        ...
        iss << line;
        iss >> opp >> mine;
        total_score += round_score(opp, mine);
        true_score += true_strat(opp, mine);
        // std::cout << "My opponent's choice: " << opp << "\tMy choice: " << mine << std::endl;
    }
    std::cout << "My total score at the end of the tournament following the strategy guide would be: " << total_score << std::endl;
    std::cout << "My total score at the end of the tournament following the correct strategy guide would be: " << true_score << std::endl;
